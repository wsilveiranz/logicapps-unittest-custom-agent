...
1. **Namespace Convention**: Use `LogicApps.Tests.<WorkflowNamePascalCase>` for test classes. **DO NOT** append `.TC01`, `.TC02`, etc. to the namespace. Example: `LogicApps.Tests.LaCalculateDiscount` (not `LogicApps.Tests.LaCalculateDiscount.TC01`).

2. **ActionMock Constructors**: Use separate constructors for success (with `outputs`) vs failure (with `error`). **NEVER** pass outputs to a failed action mock.

3. **ServiceProvider Output Format**: Must include exactly these properties with `[JsonProperty]` attributes:
   - `body` (JToken)
   - `statusCode` (int) 
   - `headers` (JObject)
...
**AGENT MUST use** the following namespace pattern for all test classes:

```csharp
namespace LogicApps.Tests.<WorkflowNamePascalCase>
```

### Namespace Rules:
1. **Base namespace**: Always `LogicApps.Tests`
2. **Workflow suffix**: Convert workflow folder name to PascalCase (remove hyphens, capitalize each word)
3. **NO test case suffix**: Do NOT append `.TC01`, `.TC02`, etc. to the namespace

### Conversion Examples:
| Workflow Folder | Namespace |
|-----------------|-----------|
| `la-calculate-discount` | `LogicApps.Tests.LaCalculateDiscount` |
| `la-receive-order` | `LogicApps.Tests.LaReceiveOrder` |
| `la-sb-process-order` | `LogicApps.Tests.LaSbProcessOrder` |

### MockOutputs Namespace:
**DO NOT modify** MockOutputs namespaces. These are pre-generated by the scaffolding tool and should remain unchanged. Test classes should import the MockOutputs namespace as-is via `using` statements.

### Example:
```csharp
// ✅ CORRECT - workflow namespace without TC suffix
namespace LogicApps.Tests.LaCalculateDiscount
{
    [TestClass]
    public class TC01_NARegionDiscount { }
}

// ❌ WRONG - includes TC suffix in namespace
namespace LogicApps.Tests.LaCalculateDiscount.TC01
{
    [TestClass]
    public class TC01_NARegionDiscount { }
}
